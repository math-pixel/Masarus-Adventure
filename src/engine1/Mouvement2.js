...
function canPlayerMove2(playerRect, Direction){

    // get x y player
    // get all x y collision block
    // 
    ...
    rectCollisionArray.forEach(currentRectCollision => {
        
        
        if (rectIsInRect( playerRect , currentRectCollision )) {
            
            // console.log(currentRectCollision)

            switch (Direction) {
                case "TOP":
                    
                    if (playerRect[1] < currentRectCollision[1]) {
                        playerCanMove = true
                    }else{
                        playerCanMove = false
                    }
                ...
                case "RIGHT":
                    ...
                    if (playerRect[0] + playerRect[2] > currentRectCollision[0]) {
                        playerCanMove = true
                    }else{
                        playerCanMove = false
                    }
                ...
                case "LEFT":
                    ...
                    if (playerRect[0] < currentRectCollision[0] + currentRectCollision[2]) {
                        playerCanMove = true
                    }else{
                        playerCanMove = false
                    }
                ...
                case "BOTTOM":
                    ...
                    if (playerRect[1] + playerRect[3] > currentRectCollision[1]) {
                        playerCanMove = true
                    }else{
                        playerCanMove = false
                    }
...
function moveMap() {
    ...
    if ( (keyIsDown(LEFT_ARROW) || keyIsDown(81)) && LEFTMap && canPlayerMove2(rectBoncingPlayer,"LEFT")) {
        xStartWorld1 += speedMoveMap;    
    }
    
    if ((keyIsDown(RIGHT_ARROW) || keyIsDown(68)) && RIGHTMap && canPlayerMove2(rectBoncingPlayer,"RIGHT")) {
        xStartWorld1 -= speedMoveMap;
    }
    
    if ((keyIsDown(UP_ARROW) || keyIsDown(90)) && TOPMap && canPlayerMove2(rectBoncingPlayer,"TOP")) {
        yStartWorld1 += speedMoveMap;
    }
    
    if ((keyIsDown(DOWN_ARROW) || keyIsDown(83))  && BOTTOMMap && canPlayerMove2(rectBoncingPlayer,"BOTTOM")) {
        yStartWorld1 -= speedMoveMap;
    }
...
function Move(){
    ...
    // cam touch border of map
    canMoveMap()

    // move map for illusion of player move
    moveMap()
